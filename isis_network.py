# -*- coding: utf-8 -*-
"""
ISIS Network Simulator
Connects multiple ISIS routers and simulates network
"""
import time
...
class ISISNetwork:
    """Network containing multiple ISIS routers"""
    
    def __init__(self):
        self.routers: Dict[str, ISISRouter] = {}
        self.links: List[tuple] = []  # [(router1, router2, interface1, interface2)]
    
    def add_router(self, system_id: str, level: ISISLevel = ISISLevel.LEVEL_1) -> ISISRouter:
        """Add router to network"""
        router = ISISRouter(system_id, level)
        self.routers[system_id] = router
        return router
    ...
    def connect_routers(self, router1_id: str, router2_id: str, 
        ...
        # Update LSPs with neighbor connections
        router1.lsdb[router1.system_id].links.append({
            'interface': interface1,
            'ip': ip1,
            'cost': cost,
            'neighbor_system_id': router2_id
        })
        router2.lsdb[router2.system_id].links.append({
            'interface': interface2,
            'ip': ip2,
            'cost': cost,
            'neighbor_system_id': router1_id
        })
        ...
        # Exchange hellos
        router1.receive_hello(router2_id, interface1, router2.level)
        router2.receive_hello(router1_id, interface2, router1.level)
        
        print(f"Connected {router1_id} <-> {router2_id} (ISIS)")
    ...
    def simulate_link_failure(self, router1_id: str, router2_id: str):
        ...
        if router1_id in self.routers and router2_id in self.routers:
            ...
            print(f"ISIS Link failure: {router1_id} <-> {router2_id}")
